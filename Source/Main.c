...
Device *device_create (Vulkan *vk);
Device *device_init (Device *device, Vulkan *vk);
...
Surface *surface_create (Vulkan *vk, Device *device, XwWindow *xw_win);
...
void     surface_destroy (Surface *surf, Device *device, Vulkan *vk);
...
int main() {
    ...
    while (is_running) {
        ...
        VkResult res = vkWaitForFences (device->device, ARRAY_SIZE (fences), fences, True, 1e9);
        GOTO_HANDLER_IF (
            res != VK_SUCCESS,
            DRAW_ERROR,
        ...
        if (resized || res == VK_SUBOPTIMAL_KHR || res == VK_ERROR_OUT_OF_DATE_KHR) {
            GOTO_HANDLER_IF (
                !surface_recreate_swapchain (surface, device, win),
                DRAW_ERROR,
        ...
        res = vkResetFences (device->device, ARRAY_SIZE (fences), fences);
        ...
        vkCmdBindPipeline (cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, ui_pipeline->pipeline);
        vkCmdBindDescriptorSets (
            ...
            0,                                            /* first set */
            1,                                            /* num sets */
            (VkDescriptorSet[]) {ui_srb->descriptor_set}, /* sets */
            0,
            Null
        );
        vkCmdBindVertexBuffers (cmd, 0, 1, (VkBuffer[]) {vbo->buffer}, (VkDeviceSize[]) {0});
        vkCmdDraw (cmd, ARRAY_SIZE (triangle_vertices), 1, 0, 0);
        ...
        res = vkQueueSubmit (device->graphics_queue.handle, 1, &submit_info, surface->render_fence);
        GOTO_HANDLER_IF (
            res != VK_SUCCESS,
            DRAW_ERROR,
        ...
        res = vkQueuePresentKHR (device->graphics_queue.handle, &present_info);
...
static inline Device *device_select_gpu (Device *device, Vulkan *vk);
...
Device *device_create (Vulkan *vk) {
    ...
    Device *idev = device_init (dev, vk);
    GOTO_HANDLER_IF (!idev, DEVICE_INIT_FAILED, ERR_OBJECT_INITIALIZATION_FAILED);
 ...
 * */
Device *device_init (Device *device, Vulkan *vk) {
    RETURN_VALUE_IF (!device || !vk, Null, ERR_INVALID_ARGUMENTS);

    /* select gpu, get memory properties, get graphics queue family index */
    device_select_gpu (device, vk);
...
/**
 * @b De-initialize and deallocate given @c Device object.
 *
 * @param device
 * */
void device_destroy (Device *device) {
 ...
 * @return @c Null otherwise.
 * */
Surface *surface_create (Vulkan *vk, Device *device, XwWindow *win) {
    RETURN_VALUE_IF (!vk || !device || !win, Null, ERR_INVALID_ARGUMENTS);

    Surface *surf = NEW (Surface);

    Surface *isurf = surface_init (surf, device, vk, win);
    GOTO_HANDLER_IF (!isurf, INIT_FAILED, ERR_OBJECT_INITIALIZATION_FAILED);

    return isurf;

INIT_FAILED:
    surface_destroy (surf, device, vk);
    return Null;
}
 ...
 * */
void surface_destroy (Surface *surf, Device *device, Vulkan *vk) {
    RETURN_IF (!surf || !device || !vk, ERR_INVALID_ARGUMENTS);

    surface_deinit (surf, device, vk);
    FREE (surf);
}
...
/**************************************************************************************************/
/******************************* DEVICE PRIVATE METHOD DEFINITIONS ********************************/
/**************************************************************************************************/

/**
 * @b Selects a GPU and gets queue family indices from the GPU.
 *
 * @param device @c Device object store selected GPU handle in.
 * @param vk @c Vulkan state used to query GPU handles.
 *
 * @return @c device on success.
 * @return @c Null otherwise.
 * */
static inline Device *device_select_gpu (Device *device, Vulkan *vk) {
    ...
    RETURN_VALUE_IF (!vk->gpu_count, Null, "Host has no usable GPU\n");
